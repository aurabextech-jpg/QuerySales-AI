...
import type { ChatResponse, ChatStep } from "@/lib/types";
  ...
  return (
    <div className={cn("animate-rise flex gap-3", isUser && "justify-end")}>
      ...
      <div className={cn("min-w-0", isUser ? "max-w-[80%]" : "flex-1")}>
        <div
          className={cn(
            "rounded-xl px-3.5 py-2.5 text-sm wrap-break-word whitespace-pre-wrap",
            isUser
              ? "bg-muted text-fg"
              : turn.failed
                ? "border-[0.5px] border-danger/30 bg-danger-tint text-danger"
...
export function AgentChat() {
  ...
  return (
    <Card className="flex h-[calc(100vh-11rem)] flex-col gap-0 overflow-hidden p-0">
      {/* Header */}
